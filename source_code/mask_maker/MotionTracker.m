        ...
        function setup(obj,filename)
            ...
            if (obj.fftXCorr)
                ...
                if (obj.spaceBlur > 0)
                    ...
                    obj.spaceBlurWght(1:obj.maxHeight,1:obj.maxWidth) = fft2(blur);
                end
            else
                if (obj.spaceBlur > 0)
            ...
            if (obj.index > (obj.frames - obj.currentOffset))
                ...
                if (obj.bidirectional)
                    ...
                    if(obj.frameParity)
                        ...
                        obj.mtnBuffer = circshift(obj.mtnBuffer,1,3);
                        obj.mtnBuffer(:,:,1) = obj.processROIs(double(obj.buffer{obj.mainChannel}(:,:,1)),(obj.currentOffset+obj.index));
                    end
                else
                    for ch = 1:obj.channels
                        if (obj.firstFrame)
        ...
        function [xDisp,yDisp] = calculateDisp(obj)
            if (obj.frameParity)
                ...
                if (obj.fftXCorr)
                    ...
                    if (obj.spaceBlur > 0)
                        adj_corr = real(ifft2(obj.fftRefImage.*fft2(obj.corrBase).*obj.spaceBlurWght));
                    else
    ...
    methods (Access = private)
        ...
        function buildRef(obj)
            ...
            if (obj.fftXCorr)
                ...
                if (obj.spaceBlur > 0)
                    obj.fftRefImage = obj.fftRefImage .* obj.spaceBlurWght;
                end
            else
                if (obj.spaceBlur > 0)                
