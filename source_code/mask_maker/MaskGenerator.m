    ...
    methods
        ...
        function thresholdMask(obj)
            if (obj.threshold < 0)
                ...
                obj.baseMask = imbinarize(((obj.image-minVal)/(max(obj.image,[],'all')-minVal)),'adaptive','ForegroundPolarity','bright','Sensitivity',sensitivity);
            else
                obj.baseMask = obj.image > obj.threshold;
            end
            obj.excludedMask = 0*obj.excludedMask;

            if ((obj.maxHole > 1) && (obj.threshold < obj.maxImVal))
                obj.removeHoles();
            end
        end
